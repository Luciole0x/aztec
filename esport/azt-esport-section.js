import { SECTION, TAG } from '../azt-app.js'

/**
 * @typedef {Object} Coach
 * @property {string} id - Pseudo
 * @property {string[]} tags - TAG
 * @property {Contact[]} contacts - Les différents moyens de contacter un coach.
 *
...
let template = /*html*/`
	...
	<svg class="icons-definition">
		<path id="Discord" d="M 36,12 C 30,13 19,16 15,19 5,34 -2,53 1,75 6,81 26,88 26,88 l 5,-9 c -6,-2 -8,-4 -8,-4 l 2,-2 c 15,8 35,8 50,0 l 2,2 c 0,0 -2,2 -8,4 l 5,9 C 74,88 94,81 99,75 102,53 95,34 85,19 81,16 70,13 64,12 l -3,6 C 51,17 48,17 39,18 Z M 33,64 a 9,10 0 1 1 0.1,0z M 67,64 a 9,10 0 1 1 0.1,0z"></path>
		<path id="Bnet" d="M 62,25 53,24 C 25,-5 25,47 43,75 c 6,9 10,15 19,21 2,1 8,4 -2,1 C 48,93 35,75 29,60 7,7 36,-21 62,25 Z m 3,48 7,-8 C 115,55 45,15 1,39 c -3,2 1,-4 8,-7 10,-6 33,-8 53,-3 40,8 59,43 3,44 z M 22,52 26,61 C 12,99 70,75 83,21 83,4 87,15 85,28 84,35 82,44 70,61 36,107 -6,99 22,52 Z"></path>
		<path id="Xbox" d="M 25,9 C 40,-1 60,-1 75,9 57,8 50,17 50,17 50,17 43,8 25,9 Z m -9,6 C 25,15 37,28 37,28 37,28 9,61 12,79 -13,45 16,15 16,15 Z M 50,40 C 12.258176,73.410795 17,85 17,85 c 17,18 49,18 66,0 0,0 4.918601,-13.533748 -33,-45 z M 63,28 c 0,0 12,-13 21,-13 0,0 30,28 5,64 C 90,59 63,28 63,28 Z"></path>
		<path id="Steam" d="m 50,0 c 30,0 50,25 50,50 0,24 -19,50 -50,50 C 35,100 11,92 2,64 l 19,8 c 5,17 29,13 28,-4 L 66,56 C 82,56 91,38 82,26 70,11 47,20 48,37 L 36,55 c -2,-1 -6,0 -9,2 L 0,46 C 2,21 23,0 50,0 Z M 63,25 C 73,22 82,32 78,42 74,52 61,52 56,44 53,40 52,29 63,25 Z m -3,19 c -6,-7 -2,-14 4,-16 7,-2 12,4 12,9 0,8 -9,13 -16,7 z M 25,73 c 11,6 14,3 16,0 3,-5 2,-11 -10,-14 9,-3 13,4 14,6 1,4 1,11 -7,14 -4,1 -9,1 -13,-6 z"></path>
		<path id="Ankama" d="M 0,11 h 74 v 33 l 5,-5 h 21 V 92 H 26 V 58 l -5,5 H 0 Z m 82,8 H 94 V 31 H 82 Z m -70,50 9,9 -9,9 -9,-9 z M 21,21 V 40 H 47 V 82 H 79 V 63 H 53 V 21 Z"></path>
	</svg>
...
export default class AztEsportSection extends HTMLElement {
	...
	static display(route) {
		let esportData = app.cache.sections.get(SECTION.ESPORT).list
		return new AztEsportSection(
			esportData.coaches,
			esportData.teams)
	}
	
	constructor(coaches, teams) {
		super()
	...
	generatesTeams(teams, route) {
		return /*html*/`
			<section class="teams">
				${teams.map(team => this.generateTeam(team, route)).join('')}
			</section>`
	}
			...
			<div class="team" style="--bg:url(./esport/media/csgo-wallpaper.jpg);">
				...
				<section class="players">
					${team.players.map(player => /*html*/`
						<div class="player">
							<div class="player-picture" style="--offX:${player.picture[0]}; --offY:${player.picture[1]}"></div>
	 ...
	 * @return {string}
	 */
	generatecoach(coach, route) {
		return /*html*/`
		<div class="coach">
			<span class="coach-picture" src="/esport/media/coaches.webp"> </span>
			<span class="coach-pseudo"> ${coach.id} </span>
			<div class="contacts">${
